<%
    request.setAttribute("pageTitle", "Kích cỡ");
...
<div class="dashboard-container">
    ...
    <div class="category-section">
        <div class="category-header">
            <h2 class="category-title">Kích cỡ</h2>
        ...
        <div class="search-card">
            <form action="${pageContext.request.contextPath}/KichCo" method="get" class="search-form">
                ...
                <c:if test="${not empty keyword}">
                    <a href="${pageContext.request.contextPath}/KichCo" class="clear-btn">
        ...
        <table class="category-table">
            ...
            <c:forEach var="temp" items="${items}">
                <tr>
                    ...
                    <td><span class="category-name">${temp.tenKichCo}</span></td>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button type="button" class="action-btn edit-btn" title="Sửa"
                                    onclick="openEditModal('${temp.id}', '${temp.tenKichCo}', '${temp.trangThai}')">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form action="${pageContext.request.contextPath}/KichCo/delete?id=${temp.id}" method="post" style="display:inline;">
...
<div id="addCategoryModal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Thêm Mới Kích Cỡ</h3>
            <span class="close-btn" onclick="closeAddModal()">&times;</span>
        </div>
        <form action="${pageContext.request.contextPath}/KichCo/insert" method="post">
            <div class="modal-body">

                <div class="form-group">
                    <label for="tenKichCo">Tên kích cỡ</label>
                    <input type="text" id="tenKichCo" name="tenKichCo" placeholder="Nhập tên kích cỡ" required>
                </div>
...
<div id="editCategoryModal" class="modal-overlay">
    <div class="modal-content">
        ...
        <form action="${pageContext.request.contextPath}/KichCo/update" method="post">
            <div class="modal-body">
                <input type="hidden" id="editId" name="id">

                <div class="form-group">
                    <label for="editTenKichCo">Tên kích cỡ</label>
                    <input type="text" id="editTenKichCo" name="tenKichCo" value="${kichCo.tenKichCo}" placeholder="Nhập tên kích cỡ" required>
                </div>

                <div class="form-group">
                    <label for="editTrangThai">Trạng thái</label>
                    <select id="editTrangThai" name="trangthai">
                        <option value="1" ${kichCo.trangThai == 1 ? 'selected' : ''}>Hoạt động</option>
                        <option value="0" ${kichCo.trangThai == 0 ? 'selected' : ''}>Không hoạt động</option>
                    </select>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    // --- XỬ LÝ MODAL SỬA ---
    function openEditModal(id, tenKichCo, trangThai) {
        // Đổ dữ liệu từ hàng được click vào các ô input trong Modal Sửa
        document.getElementById("editId").value = id;
        document.getElementById("editTenKichCo").value = tenKichCo;
