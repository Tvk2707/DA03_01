<%
    request.setAttribute("pageTitle", "Tròng Kính");
...
<div class="dashboard-container">
    ...
    <div class="category-section">
        <div class="category-header">
            <h2 class="category-title">Tròng Kính </h2>
        ...
        <div class="search-card">
            <form action="${pageContext.request.contextPath}/TrongKinh" method="get" class="search-form">
                <div class="search-input-wrapper">
                    <i class="fas fa-search search-icon"></i>
                    <input type="text" name="keyword" value="${keyword}" placeholder="Tìm kiếm tròng kính...">
                </div>
                <button type="submit" class="search-btn">
                    <i class="fas fa-search"></i> Tìm kiếm
                </button>
                <c:if test="${not empty keyword}">
                    <a href="${pageContext.request.contextPath}/TrongKinh" class="clear-btn">
        ...
        <table class="category-table">
            ...
            <c:forEach var="temp" items="${items}">
                <tr>
                    ...
                    <td><span class="category-name">${temp.loaiTrong}</span></td>
                    ...
                    <td>
                        <div class="action-buttons">
                            <button type="button" class="action-btn edit-btn" title="Sửa"
                                    onclick="openEditModal('${temp.id}', '${temp.loaiTrong}', '${temp.trangThai}')">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form action="${pageContext.request.contextPath}/TrongKinh/delete?id=${temp.id}" method="post" style="display:inline;">
...
<div id="addCategoryModal" class="modal-overlay">
    <div class="modal-content">
        ...
        <form action="${pageContext.request.contextPath}/TrongKinh/insert" method="post">
            <div class="modal-body">
                <div class="form-group">
                    <label for="loaiTrong">Loại tròng kính</label>
                    <input type="text" id="loaiTrong" name="loaiTrong" placeholder="Nhập loại tròng kính" required>
                </div>
...
<div id="editCategoryModal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            <h3>Chỉnh Sửa Tròng Kính</h3>
            <span class="close-btn" onclick="closeEditModal()">&times;</span>
        </div>
        <form action="${pageContext.request.contextPath}/TrongKinh/update" method="post">
            <div class="modal-body">
                <input type="hidden" id="editId" name="id">

                <div class="form-group">
                    <label for="editLoaiTrong">Loại tròng kính</label>
                    <input type="text" id="editLoaiTrong" name="loaiTrong" value="${trongKinh.loaiTrong}"placeholder="Nhập tên thương hiệu" required>
                </div>
...
<script>
    ...
    // --- XỬ LÝ MODAL SỬA ---
    function openEditModal(id, loaiTrong, trangThai) {
        // Đổ dữ liệu từ hàng được click vào các ô input trong Modal Sửa
        document.getElementById("editId").value = id;
        document.getElementById("editLoaiTrong").value = loaiTrong;
