        ...
        <table class="category-table">
            ...
            <c:forEach var="coupon" items="${items}" varStatus="loop">
                <tr>
                    ...
                    <%-- Phiếu giảm giá: Mã + Tên + Loại phiếu --%>
                    <td>
                        <strong style="color: #1f2937; display: block;">${coupon.maVoucher}</strong>
                        <span style="color: #6b7280; font-size: 0.85em;">${coupon.tenVoucher}</span>
                        <span class="badge-type" style="margin-top: 4px; display: inline-block;">${coupon.loaiPhieu == 1 ? 'Cá nhân' : 'Công khai'}</span>
                    </td>

                    <%-- Giá trị: Giá trị giảm + Loại giảm --%>
                    <td>
                    ...
                    <%-- Điều kiện: Đơn tối thiểu + Số lượng còn lại --%>
                    <td>
                        <div style="font-size: 0.85em; color: #6b7280;">Đơn tối thiểu:</div>
                        <span style="font-weight: 600;"><fmt:formatNumber value="${coupon.donToiThieu}" type="number" maxFractionDigits="0"/> đ</span>
                        <div style="font-size: 0.85em; color: #6b7280; margin-top: 4px;">Còn lại:</div>
                        <span style="font-weight: 600;">${coupon.soLuongDaDung == null ? 0 : coupon.soLuongDaDung}/${coupon.soLuong}</span>
                    </td>

                    <%-- Thời gian: Ngày bắt đầu → Ngày kết thúc --%>
                    <td>
                        <div style="white-space: nowrap;">${coupon.ngayBatDauText}</div>
                        <div style="color: #9ca3af; font-size: 0.8em; margin: 2px 0;">→</div>
                        <div style="white-space: nowrap;">${coupon.ngayKetThucText}</div>
                    </td>
